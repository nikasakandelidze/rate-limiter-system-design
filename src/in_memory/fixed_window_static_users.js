...
app.all("/api", (req, resp) => {
  ...
  if (fixedWindowLimiter[id]) {
    ...
    /* Since this implementation is concerned with more static user base we don't care about removing entries from state, 
    so simply let's update the data in case of record already been present */
    if (moment(now).diff(moment(timestamp), "seconds") > TIME_PERIOD) {
